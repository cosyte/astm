 ...
 *
 * `resolve` points a snippet's `import ... from "@cosyte/astm"` at this repo's source, the same way the
...
docSnippetSuite({
  ...
  resolve: (specifier) =>
    specifier === "@cosyte/astm" ? join(import.meta.dirname, "..", "src", "index.ts") : undefined,
});
