 ...
 *     does not contain types." and exits **0**. This is the half that proves the
 *     false green is real rather than asserted: without it, every case below could
 *     pass on a wrapper that changed nothing. If a future `attw` upgrade fixes that
 ...
 *     shape the `tsup` build window takes (a `dist/` removed, or not yet written,
 *     underneath the gate).
 ...
 *  6. The refusals that keep net 2 readable. `--quiet`, `--format json`, a
 *     `.attw.json` setting either, and `--config-path` at a file setting either
 *     were each measured to make the untyped sentence unreadable and hand back
 *     exit 0, which is the exact false green this file exists to close. Note the
 *     qualifier on the last one: `--config-path` at a path that does not exist
 *     blinds NOTHING (`readConfig` swallows the `ENOENT` and the sentence still
 *     prints), so the row below points it at a real config that sets `format`.
 *     Testing it the other way would assert the refusal over an argument that was
 *     harmless, which proves the refusal fires but not that it is warranted.
 *     The refusal is still by option NAME and not by value, so the shipped guard
...
/**
 * A config file that is NOT `.attw.json`, so only the `--config-path` refusal can
 * see it. It sets a key that genuinely blinds the post-check, which is what makes
 * the refusal warranted rather than merely fired.
 */
const BLINDING_CONFIG = "blinding.json";
/**
 * Drives the wrapper's empty-transcript backstop by stubbing `spawnSync` to return
 * attw's worst case: exit 0 with nothing on either stream. Every route that could
 * produce that today is refused before attw runs, so the guard is otherwise
 * unreachable from outside, and an unreachable guard is an untested one.
 */
const EMPTY_TRANSCRIPT_PROBE =
  'require("node:child_process").spawnSync = () => ({ status: 0, stdout: "", stderr: "" });' +
...
beforeAll(() => {
  ...
  writePkg(
    ...
    },
    {
      ...
      [BLINDING_CONFIG]: JSON.stringify({ format: "json" }),
    },
  );
...
describe("the refusals that keep the post-check readable", () => {
  ...
  it.each([
    ...
    ["-f json", ["-f", "json"]],
    ["--format=json", ["--format=json"]],
    ["--config-path", ["--config-path", BLINDING_CONFIG]],
  ...
  it(
    "is right that --config-path blinds: bare attw drops the sentence and exits 0 through it",
    () => {
      // The row above asserts the refusal. This asserts the refusal is WARRANTED,
      // which is the half that a refused-by-inference guard cannot show. Without
      // it, `--config-path` could be refused over an argument that changed nothing.
      //
      // NOT silence, and the difference is load-bearing here: this route sets
      // `format`, so attw prints a full JSON document (carrying `"types": false`)
      // and merely OMITS the untyped sentence. The wrapper has a different net for
      // each case, and only the post-check is what this route defeats. The
      // `.attw.json {quiet:true}` case below is the one that is literally silent.
      const bare = run(
        ATTW_BIN,
        ["--pack", ".", ...OFFLINE, "--config-path", BLINDING_CONFIG],
        typesNotPacked,
      );
      expect(bare.code).toBe(0);
      expect(bare.out).not.toContain(UNTYPED);
  ...
  it(
    "fails when attw exits 0 having printed nothing at all",
    () => {
      // The backstop under net 2: if some route not on the BLINDING list ever
      // silences attw, an empty transcript must not read as a pass. Exercised by
      // calling the wrapper's own guard, because every route that produces an
      // empty transcript today is refused before attw ever runs.
      const r = run(process.execPath, ["-e", EMPTY_TRANSCRIPT_PROBE, WRAPPER], typesNotPacked);
      expect(r.code).not.toBe(0);
