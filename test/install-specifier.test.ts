...
describe("the documented install specifier", () => {
  for (const doc of ["docs-content/installation.md", "README.md"]) {
    it(`AC-AS6: every install command in ${doc} names package.json name`, () => {
  ...
  it("AC-AS6: a specifier that is not the package name is read as the name it prints", () => {
    expect(installSpecifiers("npm install @cosyte/astn\n`pnpm add -D @cosyte/astm@0.0.1`")).toEqual(
