/**
 * Public entry point for the `@cosyte/astm` package.
 *
...
/**
 * The result of parsing a ASTM payload.
 *
...
export interface ParsedAstm {
  ...
  readonly warnings: readonly AstmWarning[];
}
...
/**
 * Options for {@link parseAstm}.
 *
 * The parser is **lenient by default** (Postel's Law): vendor quirks become {@link AstmWarning}s
 * rather than failures. Set `strict` to escalate every Tier-2 deviation to a thrown error.
 */
export interface ParseAstmOptions {
  /** When `true`, escalate every Tier-2 deviation to a thrown error instead of a warning. */
  readonly strict?: boolean;
}

/**
 * Parse a ASTM payload into a {@link ParsedAstm}.
 *
 * **Lenient by default** — real-world, vendor-quirky input parses rather than throws, emitting
 * {@link AstmWarning}s instead (Postel's Law). The complementary serializer (added in a later
 * phase) always emits spec-clean output. Only unrecoverable structural corruption throws (a Tier-3
 * {@link FatalCode}).
 *
 * @param raw - The raw ASTM payload (`string` for now; `Buffer`/`Uint8Array` support is added
 *   alongside the real parser).
 * @param options - Parse options; see {@link ParseAstmOptions}. Lenient unless `strict` is set.
 ...
 *
 * const { value, warnings } = parseAstm(raw);
 * for (const w of warnings) console.warn(w.code, w.position);
 * ```
 */
export function parseAstm(raw: string, options: ParseAstmOptions = {}): ParsedAstm {
 ...
 * @example
 * ```ts
 * import { WARNING_CODES } from "@cosyte/astm";
 *
 ...
 * @example
 * ```ts
 * import { FATAL_CODES } from "@cosyte/astm";
 *
 * try {
 *   parseAstm(raw);
 * } catch (err) {
