 ...
 *
 * Returns `undefined` for `undefined`, for `null`, and for a value that stated no
 * component at all. Never throws, whatever it is handed. The truncation flag,
 * the precision and the raw digit run are deliberately not carried over: a
 * dangling half-component was never a complete component, so it appears in no
 * field here.
 *
 ...
 * value {@link parseAstmDate} produces; the shared name is the portable route
 * and the repo-native name is unchanged.
 *
 * Returns `undefined` for `undefined`, for `null`, for a value stating no
 * component, and for one stating no year: an ASTM timestamp opens with a
 * mandatory four-digit year, so a yearless value is not one this parser reads.
 * Never throws.
 *
 * @param value - A parsed date, or the `undefined` {@link parseAstmDate} returns.
