...
The record layer is deliberately unchanged. `serializeAstmRecords` returns a string, not bytes, and all three bytes round-trip through parse and emit byte for byte with the value, units and status intact, so a consumer on a raw transport that never frames anything keeps a byte the sender genuinely supplied. `CR` and `LF` remain the record layer's own refusal, because they end a record.
