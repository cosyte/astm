...
The gate is now enforced at two points rather than one. `AstmProfile` is a plain interface, so a
hand-authored object literal type-checks and both `parseAstmRecords(raw, { profile })` and
`setDefaultAstmProfile` accept one without re-running the factory. The warning transform therefore
re-checks the gate before downgrading: a safety-critical warning is left alone whatever the profile
says. It declines rather than throwing, so a hand-authored profile still parses; it just cannot
quiet a safety-critical code.
...
`{ strict: true }` parse accept the stream. Delimiters are re-read at the same letter, so an
unrecognized header also fails to re-scope them: where the sets differ the merged tail loses its
fields rather than merely being misfiled. The parser is deliberately unchanged: inferring a
...
survivors were re-derived against both readers of record structure (message grouping and message
classification). Each is measured by comparing the same logical stream with the reported condition
present and absent, which is a comparison shown to fail for the code that was removed.
