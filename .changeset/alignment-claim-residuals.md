...
escape report ever sees a sequence to raise: a tail code fires with neither companion. Measured
exhaustively over all four role assignments, 145,800 tuples: 116,640 fire, 1,377 are orphans, and 0
of those lack `ASTM_RECORD_DELIMITER_ROLE_COLLISION`, which no profile may tolerate. The defence
...
Both corrections are stated once and pointed at rather than restated per sink, because a paraphrase
at each sink is a separate claim that goes stale on its own. The displacement lives in one paragraph
on `ShiftedFieldsSink` and the companion scope in one paragraph on the tail test; the other sinks,
the three warning factories, the profile safety gate, `README.md` and the troubleshooting guide point
at those. The three runtime warning messages now say "at least one place" and tell the operator to
read the raw line rather than count warnings.
